...
impl Model {
    ...
    ///
    /// - **base canon** = the repo-general canon sections (§10, §15–§17, §22–§24) + the
    ///   create-project scaffold dimensions;
    /// - **`cli` profile** = the 17 CLI-surface canon sections (the §1–§24 sections not rooted
    ///   in base); together with base, `cli` resolves to the full §1–§24;
    ...
    /// profile's canon sections. For `cli` this is the full `1..=24`.
...
mod tests {
    ...
    fn base_canon_holds_the_seeded_sections_and_scaffold_dims() {
        ...
        for section in [10u8, 15, 16, 17, 22, 23, 24] {
    ...
    #[test]
    fn cli_profile_plus_base_covers_all_of_1_to_24() {
        let model = Model::standard();
        assert_eq!(
