...
This project follows the AI-first CLI conventions — strict input validation, `--json` output, \
JSONL logs, no interactive prompts, informative errors, and composable commands. Use the \
`/ai-first-cli-canon` skill installed by `project-canon skill install` whenever designing or changing \
CLI surface. The installed, versioned skill is authoritative; do not maintain a repo-local canon copy.\n\n\
...
Generated by `project-canon new`. Each row is a resolved section of the AI-first CLI canon \
provided by the `/ai-first-cli-canon` skill for the `{}` profile. Flip `todo` → `pass` (with evidence) \
as the surface lands; `n/a` rows switch on when their gating question becomes yes. Verify \
mechanically with `project-canon doctor`.\n\n\
...
mod tests {
    ...
    fn base_scaffold_has_the_invariant_files() {
        ...
        assert!(has_file(&plan, "AGENTS-AI-FIRST-CLI.md").is_none());
        let agents = &has_file(&plan, "AGENTS.md").unwrap().content;
        assert!(agents.contains("/ai-first-cli-canon"));
        assert!(agents.contains("project-canon skill install"));
        let conformance = &has_file(&plan, "CONFORMANCE.md").unwrap().content;
        assert!(conformance.contains("/ai-first-cli-canon"));
        assert!(!conformance.contains("AGENTS-AI-FIRST-CLI.md"));
    }
